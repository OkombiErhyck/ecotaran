...
import { UserContext } from './UserContext'; // Assuming you have a UserContext for managing user authentication

import './OrdersPage.css';

const OrdersPage2 = () => {
  const { user } = useContext(UserContext); // Assuming you have access to the logged-in user
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`/orders?userId=${user.id}`); // Assuming the API supports filtering orders by user ID
